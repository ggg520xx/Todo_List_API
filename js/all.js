...
function addTodo() {
  ...
  if (todo.txt != "") {
    ...
    axios
      .post("https://fathomless-brushlands-42339.herokuapp.com/todo5", todo)
      .then(function (response) {
        // post 新資料上去後 必須要get render
        axios
          .get("https://fathomless-brushlands-42339.herokuapp.com/todo5")
          .then(function (response) {
            console.log(response.data);
...
function deleteAndChecked(e) {
    ...
    todoData.forEach(function (item, index) {
      if (item.id == id) {
        ...
        if (todoData[index].checked == "checked") {
          ...
          axios
            .patch(
              `https://fathomless-brushlands-42339.herokuapp.com/todo5/${id}`,
              todoData[index]
            )
            .then(function (response) {
              // console.log(response.data);
  ...
  // render(); // 可以做刪除了 Render掛上 執行重新渲染畫面
  updateList();
}
...
function updateList() {
  // let showData = [];  改Api寫法用不到了
  let str = ""; // 在此處api因素不好另推一個陣列進render 因此在此函式內依條件重組字串html結構的str串

  // 上面寫切換tab時展現css 有去抓出目前點到位置狀態
  // 如當前被賦予中的全域狀態變數為all之類的

  if (toggleStatus == "all") {
    // showData = todoData;   改Api寫法用不到了
    render();
  ...
  // render();  改Api寫法用不到了
  // 原生JS難怪可以把這個 updateList 寫在各處結尾處 取代render 原來是因為這函式有連帶呼叫render 去更新待完成的數量
}
...
clearFin.addEventListener("click", function (e) {
  ...
  const finished = todoData.filter((item) => item.checked == "checked");
  finished.forEach(function (item, index) {
    ...
    axios
      .delete(
        `https://fathomless-brushlands-42339.herokuapp.com/todo5/${item.id}`
      )
      .then(function (response) {});
